...
**写入相册**：将合成后的图片通过媒体库 API 保存到用户相册中。
...
- ✅ **手势操作**：支持双指缩放和单指拖拽平移画布，双击屏幕图片回到中央
  - 通过触摸手势计算缩放因子和平移距离
  - 限制缩放范围在0.3x到10x之间
  - 实现了惯性滑动和边界限制

- ✅ **比例裁剪**：提供多种固定裁剪比例，并允许用户调整裁剪框，以及单击裁剪框回正
...
## 复盘
复盘下来来，UI部分的基本实现比较轻松，可能最大的挑战就是适配不同设备的显示比例，以及有无导航栏挡住按钮之类的。但是想要让UI显得美观，同时有交互感，就要反复调整颜色，位置，样式等。
本次训练营课题作业最大的难点就是编辑页的业务逻辑，从0到1学习OpenGL的内容，以及把它应用在项目中。由于起初没有经验，我在设计ImageEditRender和ImageEditView时没有留下太多余地，没有设计分离的设计类和数据类，留足接口，而是简单地实现了裁剪的逻辑。但是当我开始加一些更多的内容，包括：裁剪框移动限制，旋转功能，滤镜，调整等越来越多的功能，我才发现了架构的重要性。
